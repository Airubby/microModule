     ...
     if(type=="cabinet"){
        geometry=new THREE.BoxGeometry(
         ...
         var geometryleft=new THREE.BoxGeometry(
                 ...
                 1.66,
             1);    
         var meshleft=new THREE.Mesh(geometryleft,materail.clone());
  ...
  var mesh2=new THREE.Mesh(geometry.clone(),materail.clone());//创建新的网格对象
   ...
   var materials2=new THREE.MeshBasicMaterial(this.mapData.materials[this.mapData.materials.length-1]);
    ...
    addDom(txt1,txt2,txt3,cube) {
        ...
        addDivDom.classList = 'tap';
        addDivDom.innerHTML = '<span style="color:white;font-size: 10px;padding: 5px">' + txt1 + '</span>'+'<span style="font-size: 11px;font-weight: bold" id="'+cube.uuid+'_value">'+txt2+'</span>'+'<p style="padding: 5px;margin-top: -3px;">'+txt3+'</p>';
        this.divIdArray.push(addDivDom.id);
    }
     initLabe(){
        for(let i=0;i< this.group.children.length;i++) {
            let divid=this.group.children[i].uuid+"_text";
            ...
            if(div!=null){
                ...
                console.log(this.group.children[i])
                div.style.left=this.transPosition(this.group.children[i].position).x + 'px';
                let posi=new THREE.Vector3(this.group.children[i].position.x,(this.group.children[i].position.y)*2,this.group.children[i].position.z);
                div.style.top=this.transPosition(posi).y-100 + 'px';
            }
        }
   ...
   // }
    //只显示最近点击的楼的标签 其他标签隐藏不显示
     cubeLabe(clickmesh) {
        if(clickmesh!=null)
        {
             if(clickmesh instanceof Array){

             }
             let cube=clickmesh;
             let divid=cube.uuid+"_text";
             for(let i=0;i< this.divIdArray.length;i++) {
                 let id =  this.divIdArray[i];
                 let div = document.getElementById(id);
                 if(id==divid)
                 {
                     if(div!=null&&div!=undefined){
                         div.style.display="";
                         div.style.left=this.transPosition(cube.position).x + 'px';
    ...
    updatevalue(uuid,value){
      let dom= document.getElementById(uuid+"_value");
      if(dom!=null){
        dom.innerHTML=value;
      }
    }
